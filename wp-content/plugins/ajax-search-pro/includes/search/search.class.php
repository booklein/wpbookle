...
if (!class_exists('wpdreams_search')) {
    ...
    abstract class wpdreams_search {
        ...
        /**
         * @var array of string functions to be used. Prevents repetitive usage of function_exists() in code
         */
        protected $mb_f = array(
            "fn_strpos" => "strpos",
            "fn_substr" => "substr",
            "fn_strlen" => "strlen",
            "fn_strtolower" => "strtolower"
        );
        ...
        function __construct($params) {
            ...
            $options['do_group'] = ($params['data']['resultstype'] == 'vertical' && $options['dont_group'] != true) ? true : false;
            ...
            // Check if mb functions are available
            foreach ($this->mb_f as $k => $v) {
                if ( function_exists("mb_" . $v) )
                    $this->mb_f[$k] = "mb_" . $v;
            }
        ...
        public function search($keyword) {
            ...
            // In case of index table, remove apostrophes
            if (w_isset_def($this->params['data']['search_engine'], 'regular') != 'regular') {
                $keyword = trim( str_replace( array('"', "'"), " ", $keyword ) );
            }

            $keyword_rev = $this->reverseString($keyword);

            $this->s = $this->escape( $keyword );
            $this->sr = $this->escape( $keyword_rev );

            // Avoid double escape, explode the $keyword instead of $this->s
            $this->_s = $this->escape( array_unique( explode(" ", $keyword) ) );
            $this->_sr = $this->escape( array_unique( array_reverse( explode(" ", $keyword_rev ) ) ) );
