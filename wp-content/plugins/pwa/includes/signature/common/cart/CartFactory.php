	...
	if(CBA.jQuery) {
		...
		new CBA.Widgets.StandardCheckoutWidget({ 
			...
			buttonSettings: { size: '[PWA_BTN_SIZE]', 
					  color: '[PWA_BTN_COLOR]', 
					  background: '[PWA_BTN_BKGD]' 
					}
			, orderInput: { format: '[CART_TYPE]', value: '[CART_VALUE]' }
		}).render('cbaButton'); });

		if (typeof showHidePlaceOrder_PWAButton !== 'undefined' && CBA.jQuery.isFunction(showHidePlaceOrder_PWAButton)) {
			showHidePlaceOrder_PWAButton(CBA.jQuery('input:radio[name=payment_method]:checked').val());
		}

		CBA.jQuery(\"input:radio[name=payment_method]\").attr(\"onClick\",\"showHidePlaceOrder_PWAButton(this.value)\");
	}
	</script>";
